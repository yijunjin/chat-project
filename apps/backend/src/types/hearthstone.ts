...
export interface HearthstoneRecommendRequest {
  ...
  goal: 'tempo' | 'survival' | 'burst' | 'value';
  notes?: string;
...
export interface HearthstoneTurnOptionsRequest {
...
export interface HearthstoneCardsByIdsRequest {
  ids: string[];
  locale?: string;
  pageSize?: number;
}

export interface HearthstoneCardDetailRequest {
  cardSlug?: string;
  cardId?: string;
  cardName?: string;
  cardType?: string;
...
export interface HearthstoneMetadataRequest {
  locale?: string;
}
...
/** Internal entity snapshot, tags keyed by tag name */
...
/** Payload broadcast over SSE on every state change */
